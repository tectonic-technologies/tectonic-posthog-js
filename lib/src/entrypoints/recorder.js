...
var isResourceTiming = function (entry) {
...
function initPerformanceObserver(cb, win, options) {
    ...
    if (options.recordInitialRequests) {
        var initialPerformanceEntries = win.performance.getEntries().filter(function (entry) {
            return (
...
function shouldRecordBody(_a) {
    var type = _a.type,
        recordBody = _a.recordBody,
        headers = _a.headers
    function matchesContentType(contentTypes) {
        var contentTypeHeader = Object.keys(headers).find(function (key) {
            return key.toLowerCase() === 'content-type'
        })
        var contentType = contentTypeHeader && headers[contentTypeHeader]
        return contentTypes.some(function (ct) {
            return contentType === null || contentType === void 0 ? void 0 : contentType.includes(ct)
        })
    }
    ...
    return __awaiter(this, arguments, void 0, function (win, initiatorType, url, start, end, attempt) {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 2:
                    return [2 /*return*/, performanceEntry]
            }
        })
    })
}
...
function _tryReadXHRBody(_a) {
    ...
    var _b = isHostOnDenyList(url, options),
        hostname = _b.hostname,
        isHostDenied = _b.isHostDenied
    if (isHostDenied) {
...
function prepareRequest(_a) {
    ...
    var entry = _a.entry,
        method = _a.method,
        status = _a.status,
        networkRequest = _a.networkRequest,
...
function _checkForCannotReadResponseBody(_a) {
    ...
    var contentTypeIsDenied = contentTypePrefixDenyList.some(function (prefix) {
        return contentType === null || contentType === void 0 ? void 0 : contentType.startsWith(prefix)
    })
    if (contentType && contentTypeIsDenied) {
        return 'Content-Type '.concat(contentType, ' is not supported')
    }
    var _c = isHostOnDenyList(url, options),
        hostname = _c.hostname,
        isHostDenied = _c.isHostDenied
    if (isHostDenied) {
        return hostname + ' is in deny list'
    }
    return null
...
function _tryReadBody(r) {
    ...
    return new Promise(function (resolve, reject) {
        var timeout = setTimeout(function () {
            return resolve('[SessionReplay] Timeout while trying to read body')
        }, 500)
        try {
            r.clone()
                .text()
...
function initNetworkObserver(
    callback,
    win, // top window or in an iframe
    options
) {
