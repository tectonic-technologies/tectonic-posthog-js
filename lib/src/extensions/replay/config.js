...
var ignorePostHogPaths = function (data) {
    ...
    if (
        url &&
        url.pathname &&
        POSTHOG_PATHS_TO_IGNORE.some(function (path) {
...
function enforcePayloadSizeLimit(payload, headers, limit, description) {
    ...
    var requestContentLength =
        (headers === null || headers === void 0 ? void 0 : headers['content-length']) || estimateBytes(payload)
    ...
    if (requestContentLength > limit) {
        return (
            LOGGER_PREFIX +
            ' '.concat(description, ' body too large to record (').concat(requestContentLength, ' bytes)')
        )
    }
    return payload
}
...
function scrubPayload(payload, label) {
    ...
    each(PAYLOAD_CONTENT_DENY_LIST, function (text) {
        if (
            (scrubbed === null || scrubbed === void 0 ? void 0 : scrubbed.length) &&
            (scrubbed === null || scrubbed === void 0 ? void 0 : scrubbed.indexOf(text)) !== -1
        ) {
...
export var buildNetworkRequestOptions = function (instanceConfig, remoteNetworkOptions) {
    var config = {
        ...
        performanceEntryTypeToObserve: __spreadArray(
            [],
            __read(defaultNetworkOptions.performanceEntryTypeToObserve),
            false
        ),
        payloadHostDenyList: __spreadArray(
            __spreadArray([], __read(remoteNetworkOptions.payloadHostDenyList || []), false),
            __read(defaultNetworkOptions.payloadHostDenyList),
            false
        ),
    }
    // client can always disable despite remote options
    var canRecordHeaders =
    ...
    config.maskRequestFn = isFunction(instanceConfig.session_recording.maskCapturedNetworkRequestFn)
        ? function (data) {
              ...
              return cleanedRequest
                  ? (_c =
                        (_b = (_a = instanceConfig.session_recording).maskCapturedNetworkRequestFn) === null ||
                        _b === void 0
                            ? void 0
                            : _b.call(_a, cleanedRequest)) !== null && _c !== void 0
                      ? _c
                      : undefined
                  : undefined
          }
        : function (data) {
              return scrubPayloads(enforcedCleaningFn(data))
          }
    return __assign(__assign(__assign({}, defaultNetworkOptions), config), {
